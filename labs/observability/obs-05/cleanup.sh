...
# This lab only reads shared Istio (istio-system) and Jaeger (observability)
# installs, and creates only namespaced Telemetry/ConfigMap/Deployment/Pod
# objects inside its own namespace — no shared/cluster-scoped resources were
# modified, so no lock to release.
exit 0
